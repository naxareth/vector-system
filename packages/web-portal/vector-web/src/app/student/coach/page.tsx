    ...
    <DashboardLayout>
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 flex items-center gap-2">
              <span className="text-purple-600">✦</span> Career Intelligence
            </h1>
            <p className="text-sm md:text-base text-gray-500">AI-powered analysis of your blockchain credentials.</p>
          </div>
          <div className="flex items-center gap-3">
             {loading && <span className="text-xs text-purple-600 animate-pulse font-medium">Analyzing...</span>}
             <button onClick={() => setIsExportModalOpen(true)} className="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-xs font-medium">Export Report</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative">
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* 📊 THE CHART */}
          <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm min-h-[350px]">
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-gray-100 pt-6">
              ...
              <div className="text-center">
                ...
                <div className="text-2xl font-bold text-purple-600">{metrics.marketAlignment}</div>
              </div>
              <div className="text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
              <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">🚀 Rising Assets</h3>
            ...
            <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
              <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">⚠️ Declining Assets</h3>
              <div className="space-y-4">
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-6 bg-white rounded-xl border border-gray-200 shadow-lg flex flex-col h-[600px] overflow-hidden">
            <div className="p-4 bg-purple-700 text-white">
              <h2 className="font-bold">Vector</h2>
              <p className="text-xs text-purple-200">Career Strategist</p>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
              {messages.map((msg, i) => (
                <div key={i} className={`p-3 rounded-lg text-sm max-w-[85%] ${msg.role === 'user' ? 'ml-auto bg-purple-600 text-white' : 'bg-white border border-gray-200 text-gray-800'}`}>
                  {msg.text}
                </div>
              ))}
              {chatLoading && <div className="p-3 bg-white border rounded-lg w-fit"><span className="animate-pulse">...</span></div>}
              <div ref={messagesEndRef} />
            </div>
            <div className="p-3 border-t bg-white">
              <div className="flex gap-2 mb-2 overflow-x-auto no-scrollbar">
                {quickPrompts.map((p, i) => (
                  <button key={i} onClick={() => handleSend(p)} className="whitespace-nowrap px-3 py-1 bg-purple-50 text-purple-700 text-xs rounded-full border border-purple-100 hover:bg-purple-100">
                    {p}
                  </button>
                ))}
              </div>
              <div className="flex gap-2">
                <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="Ask anything..." className="flex-1 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500" />
                <button onClick={() => handleSend()} disabled={chatLoading} className="bg-purple-600 text-white px-3 rounded-lg">➜</button>
              </div>
            </div>
          </div>
        </div>
