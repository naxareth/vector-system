...
export default function RegistrarDashboard() {
  ...
  const fetchSchemas = async () => {
    const { data, error } = await supabase.from('credential_schemas').select('*').order('created_at', { ascending: false });
    if (data) {
      setSchemas(data);
  ...
  const handleSchemaChange = (schemaId: string) => {
    ...
    setDynamicData({}); // Reset dynamic fields when schema changes
  };
  ...
  const handleIssueCredential = async () => {
    if (!selectedStudent || !selectedSchema) return alert("Please select a student and a schema template.");
    ...
    try {
      ...
      // Using a pseudo-random ID for the token for this example, or you can implement a counter in your contract
      ...
      // 2. Post to Backend W3C Registry API
      ...
      const response = await fetch('/api/registrar/credentials', {
        ...
        body: JSON.stringify({
          ...
          skill_name: selectedSchema.title,
          credential_data: dynamicData,
      ...
      setMintingProgress({ isOpen: true, progress: 100, status: 'complete', message: 'Verifiable Credential successfully issued!', txHash: tx.hash });
    } catch (error: any) {
      setMintingProgress({ isOpen: true, progress: 0, status: 'error', message: error.message || "Minting failed" });
    }
  };

  const filteredStudents = students.filter(s => (s.full_name?.toLowerCase().includes(searchQuery.toLowerCase())) || (s.student_id?.toLowerCase().includes(searchQuery.toLowerCase()))).slice(0, 5);
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
                    {filteredStudents.map((s) => (
                      <div key={s.id} onClick={() => { setSelectedStudent(s); setSearchQuery(s.full_name); setShowDropdown(false); }} className="px-4 py-3 cursor-pointer hover:bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                        <div><p className="text-sm font-bold text-gray-900">{s.full_name}</p><p className="text-xs text-gray-500">ID: {s.student_id}</p></div>
                        {s.wallet_address ? <span className="text-[10px] bg-green-100 text-green-700 px-2 py-1 rounded font-bold">Vault Linked</span> : <span className="text-[10px] bg-red-100 text-red-700 px-2 py-1 rounded font-bold">No Vault</span>}
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    {Object.entries(selectedSchema.json_schema.properties).map(([key, fieldDetails]) => (
                      <div key={key}>
                        <label className="block text-sm font-medium text-gray-700 mb-1 capitalize">
                          {fieldDetails.title} {selectedSchema.json_schema.required?.includes(key) && <span className="text-red-500">*</span>}
                        </label>
              ...
              <div>
                ...
                <input type="text" onChange={(e) => setStaticData({...staticData, certificateNumber: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500" placeholder="e.g. W3C-2027-001" />
              </div>

              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-2 flex justify-between"><span>Confidential Remarks</span><span className="text-xs text-purple-600 bg-purple-50 px-2 py-0.5 rounded border border-purple-100 font-bold">🔒 Encrypted Storage</span></label>
                <textarea onChange={(e) => setStaticData({...staticData, privateNotes: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500" rows={3} placeholder="Internal registrar notes..." />
              </div>
            </div>

            <button onClick={handleIssueCredential} className="w-full py-4 bg-gray-900 text-white font-bold rounded-xl hover:bg-black transition-all shadow-lg flex items-center justify-center gap-2">
              Issue W3C Verified Credential
            </button>
          </div>
        )}

        {/* Issuance Progress Modal (Kept mostly identical) */}
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 text-center">
              ...
              <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden mb-6">
                <div className={`h-full transition-all duration-500 ${mintingProgress.status === 'error' ? 'bg-red-500' : 'bg-purple-600'}`} style={{ width: `${mintingProgress.progress}%` }}></div>
              </div>
              {(mintingProgress.status === 'complete' || mintingProgress.status === 'error') && (
                <button onClick={() => setMintingProgress({ isOpen: false, progress: 0, status: 'minting', message: '' })} className="w-full py-3 bg-gray-900 text-white font-bold rounded-xl">Close</button>
              )}
            </div>
          </div>
        )}
