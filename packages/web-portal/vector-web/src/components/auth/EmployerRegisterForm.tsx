  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${origin}/api/auth/callback?role=employer`,
        },
      });
      if (error) {
        setServerError('Sign up with Google failed. Please try again.');
      }
    } catch (err: unknown) {
  ...
  return (
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 hover:shadow-sm text-gray-700 font-medium text-sm transition-all active:scale-[0.98]"
      >
